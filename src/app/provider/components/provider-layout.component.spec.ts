...
describe('ProviderLayoutComponent', () => {
  ...
  it('should display the provider portal brand', () => {
    const compiled = fixture.nativeElement as HTMLElement;
    const brandLink = compiled.querySelector('.brand-link');

    expect(brandLink?.textContent?.trim()).toBe('Provider Portal');
  });
  ...
  it('should include notification bell with provider role', () => {
    const compiled = fixture.nativeElement as HTMLElement;
    const notificationBell = compiled.querySelector('app-notification-bell');

    expect(notificationBell).toBeTruthy();
    expect(notificationBell?.getAttribute('role')).toBe('provider');
  });
