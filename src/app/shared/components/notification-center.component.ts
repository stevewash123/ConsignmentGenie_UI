...
@Component({
  ...
  template: `
    <div class="notifications-container">
      ...
      <div class="notifications-header">
        ...
        <div class="header-actions">
          ...
          <button
            ...
            (click)="markAllAsRead()"
            [disabled]="!hasUnreadNotifications || loadingService.isLoading(loadingKey)">
            {{ loadingService.isLoading(loadingKey) ? 'Marking...' : 'Mark All Read' }}
          </button>
        </div>
      </div>
      ...
      <!-- Empty State -->
      <div *ngIf="!loadingService.isLoading(loadingKey) && (!notifications || notifications.length === 0)" class="empty-state">
        <div class="empty-icon">🔔</div>
        <h3>{{ showUnreadOnly ? 'No unread notifications' : 'No notifications' }}</h3>
        <p>{{ showUnreadOnly ? 'All caught up!' : "You'll see updates about your activities here." }}</p>
      </div>

      <!-- Notifications List -->
      <div *ngIf="!loadingService.isLoading(loadingKey) && notifications && notifications.length > 0" class="notifications-list">
        <div
      ...
      <div class="settings-link">
        <a [routerLink]="'/' + role + '/notifications/preferences'" class="btn btn-link">
