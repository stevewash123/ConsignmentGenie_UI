...
describe('consignorstatementDetailComponent', () => {
  ...
  beforeEach(async () => {
    ...
    mockConsignorService.regenerateStatement.and.returnValue(of(mockStatement));
  ...
  describe('without detectChanges', () => {
    ...
    it('should not download PDF when no statement', () => {
      ...
      expect(mockConsignorService.downloadStatementPdf).not.toHaveBeenCalled();
    });

    it('should not regenerate when no statement', () => {
      component.statement = null;
      component.regenerateStatement();
      expect(mockConsignorService.regenerateStatement).not.toHaveBeenCalled();
    });
  });
  ...
  describe('with detectChanges', () => {
    ...
    it('should load statement on init', () => {
      expect(mockConsignorService.getStatement).toHaveBeenCalledWith('stmt-1');
    ...
    it('should download PDF successfully', fakeAsync(() => {
      ...
      expect(mockConsignorService.downloadStatementPdf).toHaveBeenCalledWith('stmt-1');
    ...
    xit('should handle PDF download error', fakeAsync(() => {
      mockConsignorService.downloadStatementPdf.and.returnValue(throwError(() => new Error('Download failed')));
      spyOn(window, 'alert');
      spyOn(console, 'error');
    ...
    it('should regenerate statement successfully', fakeAsync(() => {
      ...
      expect(mockConsignorService.regenerateStatement).toHaveBeenCalledWith('stmt-1');
    ...
    xit('should handle regenerate statement error', fakeAsync(() => {
      mockConsignorService.regenerateStatement.and.returnValue(throwError(() => new Error('Regenerate failed')));
      spyOn(window, 'alert');
      spyOn(console, 'error');
    ...
    it('should not download PDF when already downloading', () => {
      mockConsignorService.downloadStatementPdf.calls.reset();
    ...
    it('should not regenerate when already regenerating', () => {
      mockConsignorService.regenerateStatement.calls.reset();
      mockLoadingService.isLoading.and.returnValue(true);
    ...
    xit('should handle error loading statement', fakeAsync(() => {
      mockConsignorService.getStatement.and.returnValue(throwError(() => new Error('API Error')));
      spyOn(console, 'error');
    ...
    it('should have back link to statements', () => {
      ...
      expect(backLink.getAttribute('routerLink')).toBe('/consignor/statements');
    });
